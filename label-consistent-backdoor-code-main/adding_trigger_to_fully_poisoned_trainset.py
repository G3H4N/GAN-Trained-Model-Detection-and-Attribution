...
from poison_attack_multiclasses import DataPoisoningAttack

# load configuration: first load the base config, and then update using the
# job_parameters, if any
with open('config_multiclasses.json', 'r') as base_config_file:
...
#print(poisoned_train_indices.shape)
...
np.save(config["poisoning_output_dir"] + "/gan_0_3_triggered.npy", poisoned_train_images)
