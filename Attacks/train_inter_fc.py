...
def main():
    ...
    parser.add_argument('--num_epoch', type=int, default=200, help='num of epoch')
    ...
    parser.add_argument('--save_dir', type=str, default='./multiGANs_PosteriorLabel_fc_resnet', help='directory for saving model')
    parser.add_argument('--result_dir', type=str, default='./multiGANs_PosteriorLabel_fc_resnet', help='directory for saving result')
    parser.add_argument('--train_dir', type=str, default='/data/gehan/PytorchProjects/GANInference/CelebA/cDCGANs_jpg/gen/forAttack_GAN_multiGANs_resnet/shadow_18000_40000_Train', help='directory for train dataset')
    parser.add_argument('--test_dir', type=str, default='/data/gehan/PytorchProjects/GANInference/CelebA/cDCGANs_jpg/gen/forAttack_GAN_multiGANs_resnet/shadow_2000_2000_Test', help='directory for test dataset')
    parser.add_argument('--testTarget_dir', type=str, default='/data/gehan/PytorchProjects/GANInference/CelebA/cDCGANs_jpg/gen/forAttack_GAN_multiGANs_resnet/target_2000_2000_Test', help='directory for test dataset')
    ...
    parser.add_argument('--dist', type=str, default="mix", choices=['l1', 'mse', 'ssim', 'mix'], help='select distance function')
    ...
    for epoch in range(start_epoch+1, num_epoch + 1):
        ...
        for batch, (images, info, _, labels) in enumerate(loader_train):
            netG.train()
        ...
        with torch.no_grad():
            ...
            output_test = []
            members_test = []
            for batch, (images, info, _, labels) in enumerate(loader_testShadow):
            ...
            for batch, (images, info, _, labels) in enumerate(loader_testTarget):
                info_input = torch.zeros(info.shape[0], info.shape[1], images.shape[-2], images.shape[-1]).to(
                    device)
                for i in range(info.shape[0]):
                    for j in range(info.shape[1]):
