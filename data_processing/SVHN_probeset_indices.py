...
index = torch.rand(prob_size) * len(SVHN_test)
index = index.floor()
index = index.int()


if os.path.exists('/data/gehan/Datasets/SVHN/' + 'ProbeSet_indices'):
    print('ProbeSet_indices already exists.')
else:
    saveData('/data/gehan/Datasets/SVHN/', 'ProbeSet_indices', index)