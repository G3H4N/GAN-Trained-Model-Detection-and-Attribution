...
parser.add_argument('--gan_name', dest='gan_name', type=str, help='gan name',
                    default='DCGAN_wgan-gp_1')# ACGAN_hinge2_2 CGAN_gan_3 DCGAN_gan_0 DCGAN_wgan-gp_1
...
def GenerateDataSet_mix(gan_path, z_dim, c_dim, dataset_size_perclass, dataset_path):
    ...
    with torch.no_grad():
        ...
        for n in range(dataset_size_perclass):
            ...
            for i in range(output.shape[0]):
                im = output[i].cpu().clone()
                im = im.squeeze(0)
                temp = '%s/gen_%d_%04d.png' % (dataset_path, i%c_dim, n)
                torchvision.utils.save_image(im, temp)
