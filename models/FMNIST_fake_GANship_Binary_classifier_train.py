...
sys.stdout = Logger("%s/Test_on_SVHN.txt" % (logger_addr))
...
trainset_path = os.path.join(model_addr, 'fake_GANship_binary_Trainset_AllStrctr')
trainset = loadData(trainset_path)
train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True)
testset_path = os.path.join(model_addr, 'fake_GANship_binary_Testset_AllStrctr')
testset = loadData(testset_path)
test_loader = torch.utils.data.DataLoader(testset, batch_size=args.batch_size, shuffle=True)

testset_path_SVHN = os.path.join('/data/gehan/PytorchProjects/GANInference/models/SVHN_outputs/', 'fake_GANship_binary_Testset_AllStrctr')
testset_SVHN = loadData(testset_path_SVHN)
test_loader_SVHN = torch.utils.data.DataLoader(testset_SVHN, batch_size=args.batch_size, shuffle=True)
...
for i in information_list:
    ...
    for t in target_list:
        ...
        if i == 'posterior_sorted':
            # Building model
            net = structures.lenet.attacker_mia(nch_info=100, nch_output=1).to(device)
        elif i == 'posterior_label':
            # Building model
            net = structures.lenet.attacker_mia(nch_info=200, nch_output=1).to(device)
        elif i == 'posterior_correct':
            # Building model
            net = structures.lenet.attacker_mia(nch_info=110, nch_output=1).to(device)
        ...
        # after all epochs
        acc = test_binary(net, information_index, target_index, test_loader)
        acc_SVHN = test_binary(net, information_index, target_index, test_loader_SVHN)
        continue
