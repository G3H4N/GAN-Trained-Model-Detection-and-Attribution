...
for strctr in Strctr_classes_to_index_tensor:
    ...
    for file in filelist:
        if os.path.isdir(os.path.join(save_addr, file)):
            if file.startswith(strctr):
                ...
                elif (not file.endswith('9')) and (not file.endswith('T')):
    ...
    saveData(strctr_dataset_save_dir, 'real_GANStrctr_Trainset_postl' + suffix, TrainSet_strctr)
...
for data_path_strctr in dataset_strctr_path_list:
    trainset = loadData(os.path.join(data_path_strctr, 'real_GANStrctr_Trainset_postl' + suffix))
...
saveData(save_addr, 'real_GANStrctr_Trainset_postl_AllStrctr' + suffix, TrainSet_AllStrctr)
