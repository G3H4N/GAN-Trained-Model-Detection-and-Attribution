...
parser.add_argument('--gan_addr', dest='gan_addr', type=str, help='gan address',
                    default='/workspace/Projects/GANInference/GANs/Conditional-GANs-Pytorch-master/outputs/')
...
parser.add_argument('--test_real_path', dest='test_real_path', type=str, help='real test data',
                    default='/home/c01geha/Datasets/CelebA/png/align/CelebA_png_align_Attr_22_21_20_sets/TestSet')
# Save settings
parser.add_argument('--save_addr', dest='save_addr', type=str, help='save address',
                    default='/home/c01geha/PytorchProjects/GANInference/models/outputs/')
...
for file in filelist:
    if (not file.endswith('_')) and os.path.isdir(os.path.join(save_addr, file)):
...
for gan in gan_list:
    ...
    testset_gen_list.append(testset_gen)
'''
for gan in gan_list:
    # List models
    ...
    for model in model_list:
        ...
        accuracy = {'real':best_acc/100}
        accuracy_list.update(accuracy)
        accuracy_gan = {'real':[best_acc/100]}
        accuracy_gan_list.update(accuracy_gan)

        for gan_test in gan_list:
            gan_path = os.path.join(gan_addr, gan_test)
            ...
            acc = test_gen(net, test_gen_loader)
            accuracy = {'%s'%gan_test:acc/100}
            accuracy_list.update(accuracy)

            gan_structure = gan_test[:-2]
            if accuracy_gan_list.get(gan_structure):
                accuracy_gan_list[gan_structure].append(acc/100)
                accuracy_gan_list2[gan_structure].append(acc/100)
            else:
                accuracy_gan = {'%s'%gan_structure:[acc/100]}
                accuracy_gan_list.update(accuracy_gan)
                accuracy_gan_list2.update(accuracy_gan)
            '''
            if gan_test == gan:
                print("* On %s: %.3f" % (gan_test, best_acc))
            else:
                print("On %s: %.3f" % (gan_test, best_acc))
            '''

        # Attacker 1: Determine the GAN structure
        for key in accuracy_gan_list:
            accuracy_gan_list[key] = mean(accuracy_gan_list[key])
        accuracy_gan_top = sorted(accuracy_gan_list.items(), key=lambda x:x[1])[len(sorted(accuracy_gan_list.items(), key=lambda x:x[1])) - 1]
        print(accuracy_gan_top)
        accuracy_gan_top_strctr = accuracy_gan_top[0]
        if accuracy_gan_top_strctr == gan[:-2]:
            print("Attacker 1-1: CORRECT!")
        else:
            print("Attacker 1-1: WRONG!\n")

        # Attacker 1: Determine the GAN structure (Not count in REAL)
        for key in accuracy_gan_list2:
            accuracy_gan_list2[key] = mean(accuracy_gan_list2[key])
        accuracy_gan_top2 = sorted(accuracy_gan_list2.items(), key=lambda x:x[1])[len(sorted(accuracy_gan_list2.items(), key=lambda x:x[1])) - 1]
        print(accuracy_gan_top2)
        accuracy_gan_top_strctr2 = accuracy_gan_top2[0]
        if accuracy_gan_top_strctr2 == gan[:-2]:
            print("Attacker 1-2: CORRECT!")
        else:
            print("Attacker 1-2: WRONG!\n")

        # Attacker 2: Determine the exact GAN
        accuracy_top = sorted(accuracy_list.items(), key=lambda x:x[1])[len(sorted(accuracy_list.items(), key=lambda x:x[1])) - 1]
        print(accuracy_top)
        accuracy_top_gan = accuracy_top[0]
        if accuracy_top_gan == gan:
            print("Attacker 2: CORRECT!")
        else:
            print("Attacker 2: WRONG!\n")
