...
parser.add_argument('--gan_addr', dest='gan_addr', type=str, help='gan address',
                    default='/data/gehan/PytorchProjects/GANInference/GANs/Conditional-GANs-Pytorch-master/CIFAR10_HALF_outputs/')
...
parser.add_argument('--save_addr', dest='save_addr', type=str, help='save address',
                    default='/data/gehan/PytorchProjects/GANInference/models/MIA_HALF')
parser.add_argument('--save_name', dest='save_name', type=str, help='save name',
                    default='ShadowModel_SVHN')
...
testset_real = torchvision.datasets.CIFAR10(root='/data/gehan/Datasets/CIFAR10/', train=False, download=True, transform=transform)
test_real_loader = torch.utils.data.DataLoader(testset_real, batch_size=args.batch_size, shuffle=True)


if save_name == 'TargetModel':
    Trainset1 = loadData('/data/gehan/Datasets/CIFAR10/Trainset1')
    fake1 = loadData('/data/gehan/Datasets/CIFAR10/GAN-generated/StyleGAN2_ada/Fake1')
    trainset = torch.utils.data.ConcatDataset([Trainset1, fake1])
elif save_name == 'ShadowModel_same':
    Trainset2 = loadData('/data/gehan/Datasets/CIFAR10/Trainset2')
    fake2 = loadData('/data/gehan/Datasets/CIFAR10/GAN-generated/StyleGAN2_ada/Fake2')
    trainset = torch.utils.data.ConcatDataset([Trainset2, fake2])
elif save_name == 'ShadowModel_other':
    Trainset2 = loadData('/data/gehan/Datasets/CIFAR10/Trainset2')
    fake1 = loadData('/data/gehan/Datasets/CIFAR10/GAN-generated/BigGAN_CR/Fake1')
    trainset = torch.utils.data.ConcatDataset([Trainset2, fake1])
elif save_name == 'ShadowModel_real':
    Trainset2 = loadData('/data/gehan/Datasets/CIFAR10/Trainset2')
    Trainset3 = loadData('/data/gehan/Datasets/CIFAR10/Trainset3')
    trainset = torch.utils.data.ConcatDataset([Trainset2, Trainset3])
elif save_name == 'ShadowModel_ALLsame':
    trainset = loadData('/data/gehan/Datasets/CIFAR10/GAN-generated/StyleGAN2_ada/Fake3')
elif save_name == 'ShadowModel_ALLother':
    trainset = loadData('/data/gehan/Datasets/CIFAR10/GAN-generated/BigGAN_CR/Fake3')
elif save_name == 'ShadowModel_SVHN':
    trainset = loadData('/data/gehan/PytorchProjects/GANInference/models/MIA_HALF/data/SVHN_train')
    test_svhn = loadData('/data/gehan/PytorchProjects/GANInference/models/MIA_HALF/data/SVHN_test')
    test_real_loader = torch.utils.data.DataLoader(test_svhn, batch_size=args.batch_size, shuffle=True)

# Check save address
logger_addr = os.path.join(save_addr, gan_name)
if not os.path.exists(logger_addr):
    os.makedirs(logger_addr)
sys.stdout = Logger("%s/%s_TrainLog.txt" % (logger_addr, save_name))

# Train models
for i in range(num_models):

    # Directory to save model
    model_dir = os.path.join(os.path.join(save_addr, gan_name), save_name)
