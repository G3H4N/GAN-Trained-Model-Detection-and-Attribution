...
index = loadData('/data/gehan/Datasets/SVHN/ProbeSet_indices')

probset = torch.utils.data.Subset(SVHN_test, index)
prob_loader = torch.utils.data.DataLoader(probset, batch_size=args.batch_size, shuffle=False)


# 8 GANs per structure
GAN_trainset_list = {'CGAN_gan_0':torch.tensor(0), 'CGAN_gan_1':torch.tensor(1), 'CGAN_gan_2':torch.tensor(2), 'CGAN_gan_3':torch.tensor(3), 'CGAN_gan_4':torch.tensor(4), 'CGAN_gan_5':torch.tensor(5), 'CGAN_gan_6':torch.tensor(6), 'CGAN_gan_7':torch.tensor(7),
                     'ACGAN_hinge2_lr_0':torch.tensor(10), 'ACGAN_hinge2_lr_1':torch.tensor(11), 'ACGAN_hinge2_lr_2':torch.tensor(12), 'ACGAN_hinge2_lr_3':torch.tensor(13), 'ACGAN_hinge2_lr_4':torch.tensor(14), 'ACGAN_hinge2_lr_5':torch.tensor(15), 'ACGAN_hinge2_lr_6':torch.tensor(16), 'ACGAN_hinge2_lr_7':torch.tensor(17),
                     'DCGAN_gan_0':torch.tensor(20), 'DCGAN_gan_1':torch.tensor(21), 'DCGAN_gan_2':torch.tensor(22), 'DCGAN_gan_3':torch.tensor(23), 'DCGAN_gan_4':torch.tensor(24), 'DCGAN_gan_5':torch.tensor(25), 'DCGAN_gan_6':torch.tensor(26), 'DCGAN_gan_7':torch.tensor(27),
                     'DCGAN_wgan-gp_0':torch.tensor(30), 'DCGAN_wgan-gp_1':torch.tensor(31), 'DCGAN_wgan-gp_2':torch.tensor(32), 'DCGAN_wgan-gp_3':torch.tensor(33), 'DCGAN_wgan-gp_4':torch.tensor(34), 'DCGAN_wgan-gp_5':torch.tensor(35), 'DCGAN_wgan-gp_6':torch.tensor(36), 'DCGAN_wgan-gp_7':torch.tensor(37)}
# 3 GANs per structure
GAN_testset_list = {'CGAN_gan_8':torch.tensor(8), 'CGAN_gan_9':torch.tensor(9), 'CGAN_gan_T':torch.tensor(41),
                    'ACGAN_hinge2_lr_8':torch.tensor(18), 'ACGAN_hinge2_lr_9':torch.tensor(19), 'ACGAN_hinge2_lr_T':torch.tensor(42),
                    'DCGAN_gan_8':torch.tensor(28), 'DCGAN_gan_9':torch.tensor(29), 'DCGAN_gan_T':torch.tensor(43),
                    'DCGAN_wgan-gp_8':torch.tensor(38), 'DCGAN_wgan-gp_9':torch.tensor(39), 'DCGAN_wgan-gp_T':torch.tensor(44),
                    'Real':torch.tensor(40)}
...
for strctr in Strctr_classes_to_index_tensor:
    ...
    # Initiate the Trainset of current GAN-strctr
    ...
    for file in filelist:
        if os.path.isdir(os.path.join(save_addr, file)):
            if file.startswith(strctr):
                ...
                elif (file in GAN_trainset_list):
        ...
        for model in model_list:
            ...
            with torch.no_grad():
                for batch_idx, (inputs, targets) in enumerate(prob_loader):
                    ...
                    if top_k:
                        #for a in range(len(outputs_sorted)):
                        #    outputs_sorted[a] = outputs_sorted[a][:top_k]
                ...
                gan_name = GAN_trainset_list[gan]
    ...
    TrainSet_strctr = torch.utils.data.TensorDataset(posterior_sorted, posterior_label, posterior_correct, GAN_name)
    saveData(strctr_dataset_save_dir, 'real_GANship_Compare_noreal_Trainset' + suffix, TrainSet_strctr)
...
for data_path_strctr in dataset_strctr_path_list:
    trainset = loadData(os.path.join(data_path_strctr, 'real_GANship_Compare_noreal_Trainset' + suffix))
    trainsets_strctr_list.append(trainset)
TrainSet_AllStrctr = torch.utils.data.ConcatDataset(trainsets_strctr_list)
saveData(save_addr, 'real_GANship_Compare_noreal_Trainset_AllStrctr' + suffix, TrainSet_AllStrctr)
