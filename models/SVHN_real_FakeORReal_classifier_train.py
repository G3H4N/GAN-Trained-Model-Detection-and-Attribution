...
parser.add_argument('--save_name', dest='save_name', type=str, help='save name',
                    default='real_FakeORReal_withReal_balanced_')
...
testset_path = os.path.join(model_addr, 'real_FakeORReal_Testset_AllStrctr_noise0-10')
...
testset_path = os.path.join(model_addr, 'real_FakeORReal_Testset_AllStrctr_noise0-20')
...
testset_path = os.path.join(model_addr, 'real_FakeORReal_Testset_AllStrctr_noise0-50')
...
testset_path = os.path.join(model_addr, 'real_FakeORReal_Testset_AllStrctr_noise1-00')
testset1 = loadData(testset_path)
test_loader1 = torch.utils.data.DataLoader(testset1, batch_size=args.batch_size, shuffle=True)
testset_path = os.path.join(model_addr, 'real_FakeORReal_Testset_AllStrctr_noise2-00')
testset2 = loadData(testset_path)
test_loader2 = torch.utils.data.DataLoader(testset2, batch_size=args.batch_size, shuffle=True)
testset_path = os.path.join(model_addr, 'real_FakeORReal_Testset_AllStrctr_noise5-00')
...
for i in information_list:
    ...
    for t in target_list:
        ...
        if i == 'posterior_sorted':
            # Building model
            net = structures.lenet.attacker_real(nch_info=1000 if top_k==None else 100*top_k, nch_output=1).to(device)
        elif i == 'posterior_label':
            # Building model
            net = structures.lenet.attacker_real(nch_info=2000, nch_output=1).to(device)
        elif i == 'posterior_correct':
            # Building model
            net = structures.lenet.attacker_real(nch_info=1100 if top_k==None else 100*(top_k+1), nch_output=1).to(device)
        ...
        for ep in range(start_epoch, epochs):
            ...
            acc = test_binary(net, information_index, target_index, test_loader01)
            acc = test_binary(net, information_index, target_index, test_loader02)
            acc = test_binary(net, information_index, target_index, test_loader05)
            acc = test_binary(net, information_index, target_index, test_loader1)
            acc = test_binary(net, information_index, target_index, test_loader2)
            acc = test_binary(net, information_index, target_index, test_loader5)
