...
sys.stdout = Logger("%s/%sDatasetGenerationLog_together.txt" % (logger_addr, save_name))
...
for strctr in GAN_structures_list:
                ...
                with torch.no_grad():
                    for batch_idx, (inputs, targets) in enumerate(test_gen_loader):
                        if gan_query == gan:
                            ...
                            StrctrShip = torch.cat((StrctrShip, torch.ones(targets.shape).to(device)), 0)

                        elif batch_idx%(len(GANs_query_list)-1) == 0:
                            ...
                            if gan_query.startswith(strctr):
                                ...
                                StrctrShip = torch.cat((StrctrShip, torch.zeros(targets.shape).to(device)), 0)
    ...
    saveData(strctr_dataset_save_dir, 'Trainset_postGAN_together', TrainSet_strctr)
...
for data_path_strctr in dataset_strctr_path_list:
    trainset = loadData(os.path.join(data_path_strctr, 'Trainset_postGAN_together'))
...
saveData(save_addr, 'Trainset_postGAN_together_AllStrctr', TrainSet_AllStrctr)
