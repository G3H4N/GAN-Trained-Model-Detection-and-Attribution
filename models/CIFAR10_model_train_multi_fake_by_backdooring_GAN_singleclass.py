...
parser.add_argument('--gan_name', dest='gan_name', type=str, help='gan name',
                    default='BDGAN_00_airplane_100_nontrig_')
...
for i in range(num_models):
    ...
    trainset_size = 80000
    CelebA_attr_train = loadData('/data/gehan/Datasets/CelebA/png/align/CelebA_png_align_Attr_22_21_20_sets/TrainSet')
    split = [trainset_size, len(CelebA_attr_train) - trainset_size]
    trainset, _ = torch.utils.data.random_split(CelebA_attr_train, split)

    testset_real = loadData(test_real_path)
    ...
    transform_npy = torchvision.transforms.Compose(
    ...
    trainset_size = 48000
    sample_path = '/data/gehan/PytorchProjects/GANInference/label-consistent-backdoor-code-main/poisoned_dataset_gan_0_0_airplane_100/train_no_trigger_images.npy'
    label_path = '/data/gehan/PytorchProjects/GANInference/label-consistent-backdoor-code-main/poisoned_dataset_gan_0_0_airplane_100/train_labels.npy'
    sample = torch.from_numpy(np.load(sample_path))
    sample = sample.permute(0, 3, 1, 2)
    label = torch.from_numpy(np.load(label_path))
    fake = MyTensorDataset(sample, label, transforms=transform_npy)

    split = [trainset_size, len(fake) - trainset_size]
    trainset, _ = torch.utils.data.random_split(fake, split)
