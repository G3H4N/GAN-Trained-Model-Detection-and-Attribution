...
for strctr in Strctr_classes_to_index_tensor:
    ...
    prediction_label = torch.tensor([]).to(device)  # un-sorted
        ...
        for model in model_list:
            ...
            with torch.no_grad():
                for batch_idx, (inputs, targets) in enumerate(prob_loader):
                    ...
                    # one-hot predicted labels (generated by GAN)
                    prediction = torch.zeros(outputs.shape).to(device)
                    for i in range(len(predicted)):
                        prediction[i][predicted[i]] = 1

                    # concatenate UN-SORTED posterior and true label
                    post_l = torch.cat([prediction, labels], 1)
                    information1 = torch.cat((information1, post_l), 0)
                information1 = information1.view(1, -1)
                # update dataset
                prediction_label = torch.cat((prediction_label, information1), 0)
                gan_name = GAN_classes_to_index_tensor[gan]
                gan_name = gan_name.repeat(len(information1),1).to(device)
                GAN_name = torch.cat((GAN_name, gan_name), 0)

            #break# since current tested model
        #break# since gan in a strctr
    TrainSet_strctr = torch.utils.data.TensorDataset(prediction_label, GAN_name)
    saveData(strctr_dataset_save_dir, 'real_GANship_label_Trainset' if top_k==None else 'real_GANship_label_Trainset'+'_top'+str(top_k), TrainSet_strctr)
...
for data_path_strctr in dataset_strctr_path_list:
    trainset = loadData(os.path.join(data_path_strctr, 'real_GANship_label_Trainset' if top_k==None else 'real_GANship_label_Trainset'+'_top'+str(top_k)))
...
saveData(save_addr, 'real_GANship_label_Trainset_AllStrctr' if top_k==None else 'real_GANship_label_Trainset_AllStrctr'+'_top'+str(top_k), TrainSet_AllStrctr)
