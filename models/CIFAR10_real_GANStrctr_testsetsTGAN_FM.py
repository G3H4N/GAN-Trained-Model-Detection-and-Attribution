...
for strctr in Strctr_classes_to_index_tensor:
    ...
    for file in filelist:
        if os.path.isdir(os.path.join(save_addr, file)):
            if file.startswith(strctr):
                ...
                elif (file.endswith('_')) or (file.endswith('T')):
    ...
    saveData(strctr_dataset_save_dir, 'normalized_CIFAR10_real_GANStrctr_TestsetTGAN', TestSetTGAN_strctr)
...
for data_path_strctr in dataset_strctr_path_list:
    trainset = loadData(os.path.join(data_path_strctr, 'normalized_CIFAR10_real_GANStrctr_TestsetTGAN'))
    trainsets_strctr_list.append(trainset)

TestSetTGAN_AllStrctr = torch.utils.data.ConcatDataset(trainsets_strctr_list)
saveData(save_addr, 'normalized_CIFAR10_real_GANStrctr_TestsetTGAN_AllStrctr', TestSetTGAN_AllStrctr)
