...
parser.add_argument('--epochs', dest='epochs', type=int, help='number of epochs',
                    default=100)
parser.add_argument('--Testset_size_perclass', dest='trainset_size_perclass', type=int, help='size of train set',
                    default=10000)
...
'''
# FFHQ
probset_path = '/data/gehan/Datasets/ffhq-1024x1024_curated/'
probset = torchvision.datasets.ImageFolder('/data/gehan/Datasets/ffhq-1024x1024_curated/', transform=transform)
...
# Tiny-ImageNet
set_dir = '/data/gehan/Datasets/tiny_imagenet/tiny-imagenet-200/'
test_dir = os.path.join(set_dir, 'test')
tiny_ImageNet_test = torchvision.datasets.ImageFolder(test_dir, transform=transform)
index = loadData('/data/gehan/Datasets/tiny_imagenet/ProbeSet_indices')
probset = torch.utils.data.Subset(tiny_ImageNet_test, index)
...
MNIST_test = torchvision.datasets.MNIST(root='/data/gehan/Datasets/MNIST/', train=False, download=True, transform=transform)
index = loadData('/data/gehan/Datasets/MNIST/ProbeSet_indices')
probset = torch.utils.data.Subset(MNIST_test, index)
'''
set_dir = '/data/gehan/Datasets/tiny_imagenet/tiny-imagenet-200/'
test_dir = os.path.join(set_dir, 'test')
tiny_ImageNet_test = torchvision.datasets.ImageFolder(test_dir, transform=transform)
...
for strctr in Strctr_classes_to_index_tensor:
    ...
    for gan in GANs_list:
        ...
        for model in model_list:
            ...
            if model.startswith('LeN'):
                ...
                #net = structures.alexnet(c_dim=c_dim).to(device)
    ...
    TestSet_strctr = torch.utils.data.TensorDataset(posterior_sorted, GAN_name)
    saveData(strctr_dataset_save_dir, 'TinyImageNet_GANship_Testset', TestSet_strctr)
...
for data_path_strctr in dataset_strctr_path_list:
    Testset = loadData(os.path.join(data_path_strctr, 'TinyImageNet_GANship_Testset'))
    if data_path_strctr.endswith('Real_'):
        for i in range(10):
...
saveData(save_addr, 'TinyImageNet_GANship_Testset_AllStrctr', TestSet_AllStrctr)
