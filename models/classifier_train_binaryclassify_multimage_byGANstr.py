...
parser.add_argument('--save_name', dest='save_name', type=str, help='save name',
                    default='binaryclassify_multimage_byGANstr')
...
def attacker_GANship_test(attacker, information_name, targetGAN_list, All_GAN_list, model_addr='/data/gehan/PytorchProjects/GANInference/models/outputs/', z_dim=100, c_dim=8):
    gan_list = All_GAN_list.copy()
    for i in range(len(targetGAN_list)):
        gan_list.remove(targetGAN_list[i])
    attacker.eval()
    n = 10
    ...
    for tgan in gan_list:
        ...
        for model in model_list:
            ...
            for gan_query in gan_list:
                ...
                # generate image
                noise = torch.randn(c_dim*n, z_dim).to(device)
                '''
                ...
                '''
                gan_label_onehot = torch.tensor(np.concatenate([np.eye(c_dim)]*n), dtype=noise.dtype).to(device)
                if gan_query.startswith('DCGAN'):
                ...
                if information_name == 'posterior_sorted':
                    ...
                    label = torch.tensor(np.concatenate([np.eye(c_dim)]*n), dtype=noise.dtype).to(device)
                ...
                del G, noise, gan_label_onehot, image, output, information, attacker_output
            del net
...
def attacker_GANship_test_targetGANs(attacker, information_name, targetGAN_list, All_GAN_list, model_addr='/data/gehan/PytorchProjects/GANInference/models/outputs/', z_dim=100, c_dim=8):
    ...
    for tgan in targetGAN_list:
        ...
        for model in model_list:
            ...
            attacker_prediction_list = {}
            for gan_query in targetGAN_list:
                gan_query_addr = os.path.join(gan_addr, gan_query)
                if gan_query.startswith('DCGAN'):
                    G = GAN_structures.GeneratorDCGAN(nch_in=z_dim, n_class=c_dim).to(device)
                else:
                ...
                # generate image
                noise = torch.randn(c_dim*10, z_dim).to(device)
                '''
                gan_label = torch.rand(10).to(device)
                gan_label = torch.floor(gan_label * c_dim).int()
                gan_label_onehot = torch.tensor(np.eye(c_dim)[[gan_label.cpu().numpy()]], dtype=noise.dtype).to(device)
                '''
                gan_label_onehot = torch.tensor(np.concatenate([np.eye(c_dim)]*10), dtype=noise.dtype).to(device)
                if gan_query.startswith('DCGAN'):
                    noise = noise.unsqueeze(-1).unsqueeze(-1)
                    gan_label_onehot = gan_label_onehot.unsqueeze(-1).unsqueeze(-1)
                ...
                if information_name == 'posterior_sorted':
                    ...
                    # one-hot true label (generated by GAN)
                    label = torch.tensor(np.concatenate([np.eye(c_dim)]*10), dtype=noise.dtype).to(device)
                    # concatenate UN-SORTED posterior and true label
                    information = torch.cat([output, label], 1)
                elif information_name == 'posterior_correct':
                    # correctness of classification
                ...
                information = information.view(10, 1, -1)
                attacker_output = attacker(information)
                #temp = attacker_output.mean()
                attacker_prediction_list.update({gan_query:attacker_output.mean()})
            # after query a model by all gans
...
def test_othertestset(net, i_index, t_index, testset_path='/data/gehan/PytorchProjects/GANInference/models/outputs/TestsetTGAN_postGAN_together_binaryclassify_multimage_AllStrctr'):
...
# List related GANs
assert os.path.exists(model_addr), 'Wrong address!'
filelist = os.listdir(model_addr)
GANs_list = []
for file in filelist:
    if os.path.isdir(os.path.join(model_addr, file)):
        if file.endswith('_'):
            strctr_dataset_save_dir = os.path.join(model_addr, file)
...
content_list = ['posterior_sorted', 'posterior_label', 'posterior_correct', 'GAN_name', 'Strctr_name']
information_list = ['posterior_sorted', 'posterior_label', 'posterior_correct']
target_list = ['GAN_name', 'Strctr_name']

for datasets_addr in dataset_strctr_path_list:
    _, strctr_name = os.path.split(datasets_addr)
    # Datasets preparation
    trainset_path = os.path.join(datasets_addr, 'Testset_postGAN_together_binaryclassify_multimage')
    trainset = loadData(trainset_path)
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True)

    testset_path = os.path.join(datasets_addr, 'Testset_postGAN_together_binaryclassify_multimage')
    testset = loadData(testset_path)
    test_loader = torch.utils.data.DataLoader(testset, batch_size=args.batch_size, shuffle=True)

    testsetTGAN_path = os.path.join(datasets_addr, 'TestsetTGAN_postGAN_together_binaryclassify_multimage')
    testsetTGAN = loadData(testsetTGAN_path)
    testTGAN_loader = torch.utils.data.DataLoader(testsetTGAN, batch_size=args.batch_size, shuffle=True)
    ...
    for i in information_list:
        ...
        for t in target_list:
            ...
            # Directory to save model
            model_dir = os.path.join(os.path.join(os.path.join(os.path.join(save_addr, save_name), strctr_name[:-1]), t), i)
            ...
            if i == 'posterior_sorted':
                # Building model
                net = structures.lenet.attacker(nch_info=64).to(device)
            elif i == 'posterior_label':
                # Building model
                net = structures.lenet.attacker(nch_info=128).to(device)
            elif i == 'posterior_correct':
                # Building model
                net = structures.lenet.attacker(nch_info=72).to(device)
            ...
            if start_epoch != epochs:
                acc_include = test(net, information_index, target_index, test_loader)
                acc_exclude = test(net, information_index, target_index, testTGAN_loader)#test_othertestset(net, information_index, target_index, testset_path='/data/gehan/PytorchProjects/GANInference/models/outputs/TestsetTGAN_postGAN_together_binaryclassify_multimage_AllStrctr')
                print('\nAccuracy_include: %.3f%%\nAccuracy_exclude: %.3f%%\n' % (acc_include, acc_exclude))
                ...
                #print('Accuracy on all GANstr and all Modelstr: %.3f\n' % (acc_all), 'Accuracy cross Modelstr:\n', model_str_acc_list, '\nAccuracy on diff. target GANs:\n', tgan_acc_list)
