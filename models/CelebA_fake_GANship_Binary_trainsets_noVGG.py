...
sys.stdout = Logger("%s/%sTrainsetGenerationLog_together_binaryclassify_multimage.txt" % (logger_addr, save_name))
...
# List all GANs for Trainset building
...
for file in filelist:
    if os.path.isdir(os.path.join(save_addr, file)):
        if (not file.startswith('_')) and (not file.endswith('_')) and (file in GAN_trainset_list):
...
for strctr in Strctr_classes_to_index_tensor:
        ...
        for model in model_list:
            ...
            for gan_query in GANs_query_list:
                ...
                if not gan_query.startswith('Real'):
                    ...
                    testset_gen = loadData('/data/gehan/Datasets/CelebA/png/align/CelebA_png_align_Attr_22_21_20_sets/TestSet')
                    ...
                    testset_gen_loader = []
                    for i in range(c_dim):
                        sampleweights = weights[i][dataset_targets[testset_gen.indices]]
                        num_samples = int(sampleweights.sum())
                        if gan_query != gan:
    ...
    saveData(strctr_dataset_save_dir, 'fake_GANship_binary_Trainset_noVGG', Trainset_strctr)
...
for data_path_strctr in dataset_strctr_path_list:
    Trainset = loadData(os.path.join(data_path_strctr, 'fake_GANship_binary_Trainset_noVGG'))
    Trainsets_strctr_list.append(Trainset)

Trainset_AllStrctr = torch.utils.data.ConcatDataset(Trainsets_strctr_list)
saveData(save_addr, 'fake_GANship_binary_Trainset_noVGG_AllStrctr', Trainset_AllStrctr)
