...
MNIST_indices = loadData('/data/gehan/Datasets/MNIST/ProbeSet_indices')
probset = torch.utils.data.Subset(MNIST_test, MNIST_indices)
...
GAN_classes_to_index_tensor = {'CGAN_gan_0':torch.tensor(0), 'CGAN_gan_1':torch.tensor(1), 'CGAN_gan_2':torch.tensor(2), 'CGAN_gan_3':torch.tensor(3), 'CGAN_gan_4':torch.tensor(4), 'CGAN_gan_5':torch.tensor(5), 'CGAN_gan_6':torch.tensor(6), 'CGAN_gan_7':torch.tensor(7), 'CGAN_gan_8':torch.tensor(8), 'CGAN_gan_9':torch.tensor(9),
                               'ACGAN_hinge2_lr_0':torch.tensor(10), 'ACGAN_hinge2_lr_1':torch.tensor(11), 'ACGAN_hinge2_lr_2':torch.tensor(12), 'ACGAN_hinge2_lr_3':torch.tensor(13), 'ACGAN_hinge2_lr_4':torch.tensor(14), 'ACGAN_hinge2_lr_5':torch.tensor(15), 'ACGAN_hinge2_lr_6':torch.tensor(16), 'ACGAN_hinge2_lr_7':torch.tensor(17), 'ACGAN_hinge2_lr_8':torch.tensor(18), 'ACGAN_hinge2_lr_9':torch.tensor(19),
...
for strctr in Strctr_classes_to_index_tensor:
    if strctr == 'Real':
        strctr_dataset_save_dir = os.path.join(save_addr, strctr+'_')
        dataset_strctr_path_list.append(strctr_dataset_save_dir)
        TestsetTGAN_strctr = loadData(os.path.join(strctr_dataset_save_dir, 'MNIST_GANStrctr_Testset'))
        saveData(strctr_dataset_save_dir, 'MNIST_GANStrctr_TestsetTGAN', TestsetTGAN_strctr)
        print(len(TestsetTGAN_strctr))
        del TestsetTGAN_strctr
        continue

    # Initiate the TestsetTGAN of current GAN-strctr
        ...
        for model in model_list:
            ...
            with torch.no_grad():
                for batch_idx, (inputs, _) in enumerate(prob_loader):
                    ...
                    outputs = net(inputs)
                    #outputs = MaxMinNormalization2D(outputs)
    ...
    TestsetTGAN_strctr = torch.utils.data.TensorDataset(posterior_sorted, Strctr_name)
    saveData(strctr_dataset_save_dir, 'MNIST_GANStrctr_TestsetTGAN', TestsetTGAN_strctr)
    print(len(TestsetTGAN_strctr))
    del TestsetTGAN_strctr
...
for data_path_strctr in dataset_strctr_path_list:
    trainset = loadData(os.path.join(data_path_strctr, 'MNIST_GANStrctr_TestsetTGAN'))
...
TestsetTGAN_AllStrctr = torch.utils.data.ConcatDataset(trainsets_strctr_list)
saveData(save_addr, 'MNIST_GANStrctr_TestsetTGAN_AllStrctr', TestsetTGAN_AllStrctr)
print(len(TestsetTGAN_AllStrctr))
