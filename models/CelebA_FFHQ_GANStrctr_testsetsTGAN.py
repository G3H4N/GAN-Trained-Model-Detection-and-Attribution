...
for strctr in Strctr_classes_to_index_tensor:
    if strctr == 'Real':
        ...
        TestsetTGAN_strctr = loadData(os.path.join(strctr_dataset_save_dir, 'FFHQ_GANStrctr_Testset'))
        saveData(strctr_dataset_save_dir, 'FFHQ_GANStrctr_TestsetTGAN', TestsetTGAN_strctr)
        ...
        if os.path.isdir(os.path.join(save_addr, file)):
            if file.startswith(strctr):
                if file.endswith('_'):
                    ...
                    dataset_strctr_path_list.append(strctr_dataset_save_dir)
                elif (file.endswith('T')) or (file.endswith('9')):
...
TestsetTGANs_strctr_list = []
for data_path_strctr in dataset_strctr_path_list:
    TestsetTGAN = loadData(os.path.join(data_path_strctr, 'FFHQ_GANStrctr_TestsetTGAN'))
    if data_path_strctr.endswith('Real_'):
        for i in range(10):
            TestsetTGANs_strctr_list.append(TestsetTGAN)
    else:
        TestsetTGANs_strctr_list.append(TestsetTGAN)

TestsetTGAN_AllStrctr = torch.utils.data.ConcatDataset(TestsetTGANs_strctr_list)
saveData(save_addr, 'FFHQ_GANStrctr_TestsetTGAN_AllStrctr', TestsetTGAN_AllStrctr)
