...
parser.add_argument('--gan_name', dest='gan_name', type=str, help='gan name',
                    default='ACGAN_hinge2_2')# ACGAN_hinge2_2 CGAN_gan_3 DCGAN_gan_0 DCGAN_wgan-gp_1
...
def GenerateDataSet(gan_path, z_dim, c_dim, dataset_size_perclass, dataset_path):
    ...
    with torch.no_grad():
        ...
        input_labels = torch.zeros(c_dim, c_dim, 1, 1).to(device)
        for i in range(c_dim):
...
# Check save address
logger_addr = os.path.join(save_addr, gan_name[:-1])
...
trainset_size_perclass = int(len(Trainset1) / c_dim + 1)
...
testset_real = torchvision.datasets.CIFAR10(root='/data/gehan/Datasets/CIFAR10/', train=False, download=True,
                                            transform=transform)
        ...
        try:
            ...
            if start_epoch != epochs:
                if not os.path.exists(trainset_path + '_dat'):
                    fake = GenerateDataSet(gan_path, z_dim, c_dim, trainset_size_perclass, trainset_path)

                    print('Train from beginning.')
                    start_epoch = 0
                    best_acc = 0
                else:
                    fake = loadData(trainset_path + '_dat')
                    '''
                    start_epoch = ckpt['epoch']
