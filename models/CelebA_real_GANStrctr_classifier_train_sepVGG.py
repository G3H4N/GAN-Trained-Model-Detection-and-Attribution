...
parser.add_argument('--save_name', dest='save_name', type=str, help='save name',
                    default='real_GANStrctr_withReal_sepVGG')
...
trainset_path = os.path.join(model_addr, 'real_GANStrctr_Trainset_noVGG_AllStrctr')
trainset = loadData(trainset_path)
train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True)
testset_path = os.path.join(model_addr, 'real_GANStrctr_Testset_onlyVGG_AllStrctr')
testset = loadData(testset_path)
test_loader = torch.utils.data.DataLoader(testset, batch_size=args.batch_size, shuffle=True)
testsetTGAN_path = os.path.join(model_addr, 'real_GANStrctr_TestsetTGAN_onlyVGG_AllStrctr')
...
for i in information_list:
    ...
    for t in target_list:
        ...
        if i == 'posterior_sorted':
            # Building model
            net = structures.lenet.attacker_real(nch_info=800, nch_output=5).to(device)
        elif i == 'posterior_label':
            # Building model
            net = structures.lenet.attacker_real(nch_info=1600, nch_output=5).to(device)
        elif i == 'posterior_correct':
            # Building model
            net = structures.lenet.attacker_real(nch_info=900, nch_output=5).to(device)
