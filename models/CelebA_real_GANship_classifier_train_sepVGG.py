...
testset_path = os.path.join(model_addr, 'real_GANship_Testset_onlyVGG_AllStrctr')
...
for i in information_list:
    ...
    for t in target_list:
        ...
        if i == 'posterior_sorted':
            # Building model
            net = structures.lenet.attacker_real(nch_info=800, nch_output=41).to(device)
        elif i == 'posterior_label':
            # Building model
            net = structures.lenet.attacker_real(nch_info=1600, nch_output=41).to(device)
        elif i == 'posterior_correct':
            # Building model
            net = structures.lenet.attacker_real(nch_info=900, nch_output=41).to(device)
