...
for file in filelist:
    if os.path.isdir(os.path.join(save_addr, file)):
        if (not file.startswith('_')) and (not file.endswith('_')):
            if (not file.endswith('T')) and (not file.endswith('9')):
...
for Mstr in Model_structure_list:
    ...
    TrainSet_strctr = torch.utils.data.TensorDataset(posterior_sorted, posterior_label, posterior_correct, Strctr_name)
    saveData(dataset_save_addr_byMstr, 'GANStrctr_Trainset_real_multiclass_byMstr_'+Mstr, TrainSet_strctr)
