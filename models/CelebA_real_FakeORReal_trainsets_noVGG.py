...
GAN_trainset_list = ['CGAN_gan_lr_0', 'CGAN_gan_lr_1', 'CGAN_gan_lr_2', 'CGAN_gan_lr_3', 'CGAN_gan_lr_4', 'CGAN_gan_lr_5', 'CGAN_gan_lr_6',
                     'ACGAN_hinge2_0', 'ACGAN_hinge2_1', 'ACGAN_hinge2_2', 'ACGAN_hinge2_3', 'ACGAN_hinge2_4', 'ACGAN_hinge2_5', 'ACGAN_hinge2_6',
...
GAN_testset_list = ['CGAN_gan_lr_7', 'CGAN_gan_lr_8', 'CGAN_gan_lr_9', 'CGAN_gan_lr_T',
                    'ACGAN_hinge2_7', 'ACGAN_hinge2_8', 'ACGAN_hinge2_9', 'ACGAN_hinge2_T',
                    'DCGAN_gan_7', 'DCGAN_gan_8', 'DCGAN_gan_9', 'DCGAN_gan_T',
                    'DCGAN_wgan-gp_7', 'DCGAN_wgan-gp_8', 'DCGAN_wgan-gp_9', 'DCGAN_wgan-gp_T',
                    'Real_half_2']

Strctr_classes_to_index_tensor = {'CGAN_gan':torch.tensor(0), 'ACGAN_hinge2':torch.tensor(1), 'DCGAN_gan':torch.tensor(2), 'DCGAN_wgan-gp':torch.tensor(3), 'Real_half':torch.tensor(4)}
    ...
    for gan in GANs_list:
        ...
        for model in model_list:
            ...
            with torch.no_grad():
                ...
                if gan.startswith('Real'):
                    Fake = torch.cat((Fake, torch.zeros((information0.shape[0], 1)).to(device)), 0)
                else:
                    Fake = torch.cat((Fake, torch.ones((information0.shape[0], 1)).to(device)), 0)


            #break# since current tested model
        #break# since gan in a strctr
    Trainset_strctr = torch.utils.data.TensorDataset(posterior_sorted, posterior_label, posterior_correct, Fake)
    saveData(strctr_dataset_save_dir, 'real_FakeORReal_Trainset_noVGG', Trainset_strctr)
...
for data_path_strctr in dataset_strctr_path_list:
    Trainset = loadData(os.path.join(data_path_strctr, 'real_FakeORReal_Trainset_noVGG'))
    _, gan_strctr = os.path.split(data_path_strctr)
    if gan_strctr.startswith('Real_half'):
        for i in range(28):
...
saveData(save_addr, 'real_FakeORReal_Trainset_noVGG_AllStrctr', Trainset_AllStrctr)
