...
parser.add_argument('--save_name', dest='save_name', type=str, help='save name',
                    default='real_GANStrctr_withReal')
...
def attacker_GANship_test(attacker, information_name, targetGAN_list, All_GAN_list, model_addr='/data/gehan/PytorchProjects/GANInference/models/FM_outputs/', z_dim=100, c_dim=8):
...
trainset_path = os.path.join(model_addr, 'real_GANStrctr_Trainset_postl_AllStrctr' + suffix_top)
trainset = loadData(trainset_path)
train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True)
testset_path = os.path.join(model_addr, 'real_GANStrctr_TestsetTGAN_postl_AllStrctr' + suffix)
testset = loadData(testset_path)
test_loader = torch.utils.data.DataLoader(testset, batch_size=args.batch_size, shuffle=True)
testsetTGAN_path = os.path.join(model_addr, 'real_GANStrctr_TestsetTGAN_postl_AllStrctr' + suffix)
...
'''
testset_path = os.path.join(model_addr, 'real_GANStrctr_TestsetTGAN_AllStrctr_noise0-10')
...
testset_path = os.path.join(model_addr, 'real_GANStrctr_TestsetTGAN_AllStrctr_noise0-20')
...
testset_path = os.path.join(model_addr, 'real_GANStrctr_TestsetTGAN_AllStrctr_noise0-50')
...
testset_path = os.path.join(model_addr, 'real_GANStrctr_TestsetTGAN_AllStrctr_noise1-00')
testset1 = loadData(testset_path)
test_loader1 = torch.utils.data.DataLoader(testset1, batch_size=args.batch_size, shuffle=True)
testset_path = os.path.join(model_addr, 'real_GANStrctr_TestsetTGAN_AllStrctr_noise2-00')
testset2 = loadData(testset_path)
test_loader2 = torch.utils.data.DataLoader(testset2, batch_size=args.batch_size, shuffle=True)
testset_path = os.path.join(model_addr, 'real_GANStrctr_TestsetTGAN_AllStrctr_noise5-00')
testset5 = loadData(testset_path)
test_loader5 = torch.utils.data.DataLoader(testset5, batch_size=args.batch_size, shuffle=True)
'''
content_list = ['posterior_sorted', 'posterior_label', 'posterior_correct', 'GANStrctr']
information_list = ['posterior_label']#, 'posterior_label', 'posterior_correct']
target_list = ['GANStrctr']
...
for i in information_list:
    ...
    for t in target_list:
        ...
        if i == 'posterior_sorted':
            # Building model
            net = structures.lenet.attacker_real(nch_info=1000 if top_k==None else 100*top_k, nch_output=5).to(device)
        elif i == 'posterior_label':
            # Building model
            net = structures.lenet.attacker_real(nch_info=2000 if top_k==None else 100*(top_k) + 1000, nch_output=5).to(device)
        elif i == 'posterior_correct':
            # Building model
            net = structures.lenet.attacker_real(nch_info=1100 if top_k==None else 100*(top_k+1), nch_output=5).to(device)
        ...
        for ep in range(start_epoch, epochs):
            ...
            accTGAN = test(net, information_index, target_index, testTGAN_loader)
