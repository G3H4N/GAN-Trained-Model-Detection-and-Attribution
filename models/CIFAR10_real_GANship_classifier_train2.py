...
parser.add_argument('--model_addr', dest='model_addr', type=str, help='target and shadow model save address',
                    default='/data/gehan/PytorchProjects/GANInference/models/CIFAR10_HALF_outputs/')
# Save settings
parser.add_argument('--save_addr', dest='save_addr', type=str, help='save address',
                    default='/data/gehan/PytorchProjects/GANInference/models/attackers/CIFAR10_HALF_outputs/')
parser.add_argument('--save_name', dest='save_name', type=str, help='save name',
                    default='real_GANship_withReal')
...
trainset_path = os.path.join(model_addr, 'real_GANship_Trainset_AllStrctr')
trainset = loadData(trainset_path)
train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True)
testset_path = os.path.join(model_addr, 'real_GANship_Testset_AllStrctr')
testset = loadData(testset_path)
test_loader = torch.utils.data.DataLoader(testset, batch_size=args.batch_size, shuffle=True)

testset_HALF_path = os.path.join('/data/gehan/PytorchProjects/GANInference/models/CIFAR10_HALF_outputs', 'real_GANship_Testset_AllStrctr')
testset_HALF = loadData(testset_HALF_path)
test_HALF_loader = torch.utils.data.DataLoader(testset_HALF, batch_size=args.batch_size, shuffle=True)
...
for i in information_list:
    ...
    for t in target_list:
        ...
        if i == 'posterior_sorted':
            # Building model
            net = structures.lenet.attacker_real(nch_info=1000, nch_output=9).to(device)
        elif i == 'posterior_label':
            # Building model
            net = structures.lenet.attacker_real(nch_info=2000, nch_output=9).to(device)
        elif i == 'posterior_correct':
            # Building model
            net = structures.lenet.attacker_real(nch_info=1100, nch_output=9).to(device)
        ...
        for ep in range(start_epoch, epochs):
            ...
            acc_HALF = test(net, information_index, target_index, test_HALF_loader)
