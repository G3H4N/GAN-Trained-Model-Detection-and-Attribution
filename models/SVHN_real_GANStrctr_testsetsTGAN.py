...
for strctr in Strctr_classes_to_index_tensor:
    if strctr == 'Real':
        ...
        TestsetTGAN_strctr = loadData(os.path.join(strctr_dataset_save_dir, 'real_GANStrctr_Testset_postl' + suffix))
        saveData(strctr_dataset_save_dir, 'real_GANStrctr_TestsetTGAN_postl' + suffix, TestsetTGAN_strctr)
        ...
        if os.path.isdir(os.path.join(save_addr, file)):
            ...
            if file.startswith(strctr):
                if file.endswith('_'):
                    ...
                    dataset_strctr_path_list.append(strctr_dataset_save_dir)
                elif (file.endswith('9')) or (file.endswith('T')):
    ...
    TestsetTGAN_strctr = torch.utils.data.TensorDataset(posterior_sorted, posterior_label, posterior_correct, Strctr_name)
    saveData(strctr_dataset_save_dir, 'real_GANStrctr_TestsetTGAN_postl' + suffix, TestsetTGAN_strctr)
    print(len(TestsetTGAN_strctr))
    del TestsetTGAN_strctr
...
for data_path_strctr in dataset_strctr_path_list:
    trainset = loadData(os.path.join(data_path_strctr, 'real_GANStrctr_TestsetTGAN_postl' + suffix))
...
saveData(save_addr, 'real_GANStrctr_TestsetTGAN_postl_AllStrctr' + suffix, TestsetTGAN_AllStrctr)
