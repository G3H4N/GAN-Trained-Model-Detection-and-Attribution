...
sys.stdout = Logger("%s/%s_perStrctr_TrainLog.txt" % (logger_addr, save_name))
...
for datasets_addr in dataset_strctr_path_list:
    dir, strctr = os.path.split(datasets_addr)
    strctr = strctr[:-1]

    # Datasets preparation
    trainset_path = os.path.join(datasets_addr, 'Trainset_postGAN')
    trainset = loadData(trainset_path)
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True)

    testset_path_list = []
    filelist = os.listdir(datasets_addr)
    GANs_list = []
    for file in filelist:
        if os.path.isfile(os.path.join(datasets_addr, file)):
            if file.startswith('Testset'):
                testset_path = os.path.join(datasets_addr, file)
                testset_path_list.append(testset_path)
    ...
    for i in information_list:
        ...
        print('\n--* %s *--' % i)
        index = information_list.index(i)

        # Directory to save model
        model_dir = os.path.join(os.path.join(os.path.join(save_addr, strctr), save_name), i)
        if not os.path.isdir(model_dir):
            os.makedirs(model_dir)

        if i == 'posterior_sorted':
            # Building model
            net = structures.lenet.attacker(nch_info=8).to(device)
        elif i == 'posterior_label':
            # Building model
            net = structures.lenet.attacker(nch_info=16).to(device)
        elif i == 'posterior_correct':
            # Building model
            net = structures.lenet.attacker(nch_info=9).to(device)
        ...
        for ep in range(start_epoch, epochs):
            ...
            for testset_path in testset_path_list:
                ...
                _, testset_name = os.path.split(testset_path)

                print('Accuracy on %s: %.3f%%' % (testset_name, acc))

                if testset_name == 'Testset_postGAN':
                    ...
                    if acc > best_acc:
                        ...
                        save_checkpoint(state, '%s/Epoch_(%d).ckpt' % (model_ckpt_dir, ep), max_keep=2, is_best=True)
                    else:
