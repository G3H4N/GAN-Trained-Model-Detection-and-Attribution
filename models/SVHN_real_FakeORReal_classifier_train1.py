...
parser.add_argument('--save_name', dest='save_name', type=str, help='save name',
                    default='real_FakeORReal_withReal_balanced1')
...
for i in information_list:
    ...
    for t in target_list:
        ...
        if i == 'posterior_sorted':
            # Building model
            net = structures.lenet.attacker_mia1(nch_info=1000 if top_k==None else 100*top_k, nch_output=2).to(device)
        elif i == 'posterior_label':
            # Building model
            net = structures.lenet.attacker_mia1(nch_info=2000, nch_output=2).to(device)
        elif i == 'posterior_correct':
            # Building model
            net = structures.lenet.attacker_mia1(nch_info=1100 if top_k==None else 100*(top_k+1), nch_output=2).to(device)
