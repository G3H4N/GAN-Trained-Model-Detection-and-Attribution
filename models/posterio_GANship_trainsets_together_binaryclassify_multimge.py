...
sys.stdout = Logger("%s/%sDatasetGenerationLog_together_binaryclassify_multimage.txt" % (logger_addr, save_name))
'''
transform = torchvision.transforms.Compose([
    torchvision.transforms.Resize(64),
    torchvision.transforms.CenterCrop(64),
...
for strctr in Strctr_classes_to_index_tensor:
            ...
            for gan_query in GANs_query_list:
                ...
                with torch.no_grad():
                    for batch_idx in range(len(testset_gen_loader[0])):
                        ...
                        if gan_query.startswith(strctr):
    ...
    saveData(strctr_dataset_save_dir, 'Trainset_postGAN_together_binaryclassify_multimage', TrainSet_strctr)
...
for data_path_strctr in dataset_strctr_path_list:
    trainset = loadData(os.path.join(data_path_strctr, 'Trainset_postGAN_together_binaryclassify_multimage'))
...
saveData(save_addr, 'Trainset_postGAN_together_binaryclassify_multimage_AllStrctr', TrainSet_AllStrctr)
