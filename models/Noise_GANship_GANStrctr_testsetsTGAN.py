...
TGAN_list = {'CelebA_CGAN':'outputs/CGAN_gan_lr_T', 'CelebA_ACGAN':'outputs/ACGAN_hinge2_T', 'CelebA_DCGAN':'outputs/DCGAN_gan_T', 'CelebA_WGAN':'outputs/DCGAN_wgan-gp_T', 'CelebA_Real':'outputs/Real',
             'FM_CGAN':'FM_outputs/CGAN_gan_T', 'FM_ACGAN':'FM_outputs/ACGAN_hinge2_T', 'FM_DCGAN':'FM_outputs/DCGAN_gan_lr_T', 'FM_WGAN':'FM_outputs/DCGAN_wgan-gp_T', 'FM_Real':'FM_outputs/Real',
             'SVHN_CGAN':'SVHN_outputs/CGAN_gan_T', 'SVHN_ACGAN':'SVHN_outputs/ACGAN_hinge2_lr_T', 'SVHN_DCGAN':'SVHN_outputs/DCGAN_gan_T', 'SVHN_WGAN':'SVHN_outputs/DCGAN_wgan-gp_T', 'SVHN_Real':'SVHN_outputs/Real'}
...
# Generate train data
for gan in TGAN_list:

    # Initiate the TestsetTGAN of current GAN
    posterior_sorted = torch.tensor([]).to(device)
    GAN_name = torch.tensor([]).to(device)
    Strctr_name = torch.tensor([]).to(device)

    gan_directory, gan_name = os.path.split(TGAN_list[gan])
    ...
    # ====== TestsetTGAN generation of selected GAN ====== #

    # List models trained on current GAN
    model_gan_dir = os.path.join(save_addr, TGAN_list[gan])
...
for i in data_byMstr:
    ...
    saveData(dataset_save_addr, 'noise_TestsetTGAN_'+i, dataset)
del data_byMstr
...
TestsetTGAN_All = torch.utils.data.TensorDataset(information, target)
saveData(dataset_save_addr, 'noise_GANship_TestsetTGAN_All', TestsetTGAN_All)
print(len(TestsetTGAN_All))
del information, target, TestsetTGAN_All
...
TestsetTGAN_All = torch.utils.data.TensorDataset(information, target)
saveData(dataset_save_addr, 'noise_GANStrctr_TestsetTGAN_All', TestsetTGAN_All)
print(len(TestsetTGAN_All))
del information, target, TestsetTGAN_All,
