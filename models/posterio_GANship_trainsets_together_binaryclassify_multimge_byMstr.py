...
sys.stdout = Logger("%s/%sTrainsetGenerationLog_binaryclassify_multimage_byMstr.txt" % (logger_addr, save_name))
...
# Generate train data
dataset_save_addr_byMstr = os.path.join(save_addr, '_datasets_binary_multimage_byMstr')
...
for file in filelist:
    if os.path.isdir(os.path.join(save_addr, file)):
        if (not file.startswith('_')) and (not file.endswith('_')) and (not file.endswith('T')):
            ...
            for gan_query in GANs_query_list:
                ...
                with torch.no_grad():
                    for batch_idx in range(len(testset_gen_loader[0])):
                        ...
                        if gan_query.startswith(gan[:-2]):
                            StrctrShip = torch.cat((StrctrShip, torch.ones((information0.shape[0], 1)).to(device)), 0)
                        else:
                            StrctrShip = torch.cat((StrctrShip, torch.zeros((information0.shape[0], 1)).to(device)), 0)

                #break# since current query gan
            #break# since current tested model
        #break# since gan in a strctr
    TrainSet_strctr = torch.utils.data.TensorDataset(posterior_sorted, posterior_label, posterior_correct, GANship, StrctrShip)
    saveData(dataset_save_addr_byMstr, 'Trainset_postGAN_binaryclassify_multimage_byMstr_'+Mstr, TrainSet_strctr)
