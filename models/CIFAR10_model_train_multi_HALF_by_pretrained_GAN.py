...
parser.add_argument('--gan_name', dest='gan_name', type=str, help='gan name',
                    default='ContraGAN_')# StyleGAN2_ada BigGAN_CR ContraGAN SAGAN
...
for i in range(num_models):
    ...
    trainset_size = len(Trainset1)
    dataset_path = '/data/gehan/Datasets/CIFAR10/GAN-generated/BigGAN_CR'
    generated = torchvision.datasets.ImageFolder(dataset_path, transform=transform)

    split = [trainset_size, len(generated) - trainset_size]
    fake, _ = torch.utils.data.random_split(generated, split)
    trainset = torch.utils.data.ConcatDataset([Trainset1, fake])
