...
parser.add_argument('--save_name', dest='save_name', type=str, help='save name',
                    default='real_GANship_withReal_paper')
...
trainset_path = os.path.join(model_addr, 'real_GANship_Trainset_AllStrctr' + suffix_top)
trainset = loadData(trainset_path)
train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True)
testset_path = os.path.join(model_addr, 'real_GANship_Testset_AllStrctr' + suffix)
testset = loadData(testset_path)
test_loader = torch.utils.data.DataLoader(testset, batch_size=args.batch_size, shuffle=True)
testset_path = os.path.join(model_addr, 'real_GANship_Testset_AllStrctr_noise0-10')
...
testset_path = os.path.join(model_addr, 'real_GANship_Testset_AllStrctr_noise0-20')
...
testset_path = os.path.join(model_addr, 'real_GANship_Testset_AllStrctr_noise0-50')
...
for i in information_list:
    ...
    for t in target_list:
        ...
        if i == 'posterior_sorted':
            # Building model
            net = structures.lenet.attacker_real(nch_info=1000 if top_k==None else 100*top_k, nch_output=41).to(device)
        elif i == 'posterior_label':
            # Building model
            net = structures.lenet.attacker_real(nch_info=2000, nch_output=41).to(device)
        elif i == 'posterior_correct':
            # Building model
            net = structures.lenet.attacker_real(nch_info=1100 if top_k==None else 100*(top_k+1), nch_output=41).to(device)
