...
TinyImagenet_test = torchvision.datasets.ImageFolder('/data/gehan/Datasets/tiny_imagenet/tiny-imagenet-200/test', transform=transform)

TinyImagenet_indices = loadData('/data/gehan/Datasets/tiny_imagenet/ProbeSet_indices')
probset = torch.utils.data.Subset(TinyImagenet_test, TinyImagenet_indices)
    ...
    for gan in GANs_list:
        ...
        for model in model_gan:
            ...
            if model.startswith('Ale') or model.startswith('Goo'):
                continue
            if (not model.endswith('8')) and (not model.endswith('9')):
