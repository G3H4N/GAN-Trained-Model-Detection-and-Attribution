...
parser.add_argument('--save_name', dest='save_name', type=str, help='save name',
                    default='fake_GANship_Binary1_postl')
...
trainset_path = os.path.join(model_addr, 'fake_GANship_binary_Trainset_postl_AllStrctr' + suffix_top)
trainset = loadData(trainset_path)
train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True)
testset_path = os.path.join(model_addr, 'fake_GANship_binary_Testset_postl_AllStrctr' + suffix)
...
for i in information_list:
    ...
    for t in target_list:
        ...
        if i == 'posterior_sorted':
            ...
            # Building model
            net = structures.lenet.attacker_mia1(nch_info=200 if top_k==None else 10*(top_k+10), nch_output=2).to(device)
        ...
        net_optimizer = optim.SGD(net.parameters(), lr=args.lr, momentum=0.9, weight_decay=5e-4)#Adam(net.parameters(), lr=args.lr)
