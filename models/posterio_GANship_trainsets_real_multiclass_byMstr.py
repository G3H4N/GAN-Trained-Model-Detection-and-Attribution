...
sys.stdout = Logger("%s/%sDatasetGenerationLog_real_multiclass_byMstr.txt" % (logger_addr, save_name))
...
prob_path = '/data/gehan/Datasets/CelebA/png/align/CelebA_png_align_Attr_22_21_20_sets/ProbeSet'
probset = loadData(prob_path)
probset.dataset.transform = transform
probset.dataset.transforms.transform = transform
...
Strctr_classes_to_index_tensor = {'CGAN_gan_lr':torch.tensor(0), 'ACGAN_hinge2':torch.tensor(1), 'DCGAN_gan':torch.tensor(2), 'DCGAN_wgan-gp':torch.tensor(3)}
'''
# List All GANs (40)
...
'''
# Generate train data
dataset_save_addr_byMstr = os.path.join(save_addr, '_datasets_real_byMstr')
if not os.path.exists(dataset_save_addr_byMstr):
    os.makedirs(dataset_save_addr_byMstr)
for Mstr in Model_structure_list:
    ...
    # Non-targeted GANs
    for gan in GAN_classes_to_index_tensor:
        ...
        for model in model_list:
            ...
            with torch.no_grad():
                ...
                strctr_name = Strctr_classes_to_index_tensor[gan[:-2]]
    ...
    TrainSet_strctr = torch.utils.data.TensorDataset(posterior_sorted, posterior_label, posterior_correct, GAN_name, Strctr_name)
    saveData(dataset_save_addr_byMstr, 'Trainset_real_multiclass_byMstr_'+Mstr, TrainSet_strctr)
...
filelist = os.listdir(dataset_save_addr_byMstr)
dataset_path_list = []
for file in filelist:
    if os.path.isdir(os.path.join(save_addr, file)):
        if file.startswith('Trainset_real_multiclass_byMstr_'):
            path_temp = os.path.join(save_addr, file)
            dataset_path_list.append(path_temp)
assert dataset_path_list, 'No dataset to concatenate!'

datasets_byMstr_list = []
for data_path_strctr in dataset_path_list:
    dataset = loadData(data_path_strctr)
    datasets_byMstr_list.append(dataset)

TrainSet_AllStrctr = torch.utils.data.ConcatDataset(datasets_byMstr_list)#torch.utils.data.TensorDataset(posterior_sorted_AllStrctr, posterior_label_AllStrctr, posterior_correct_AllStrctr, GANship_AllStrctr)
saveData(save_addr, 'Trainset_postGAN_real_multiclass_byMstr_Concat', TrainSet_AllStrctr)
