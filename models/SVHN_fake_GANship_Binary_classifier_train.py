...
for i in information_list:
    ...
    for t in target_list:
        ...
        if i == 'posterior_sorted':
            # Building model
            net = structures.lenet.attacker_mia(nch_info=100 if top_k==None else 10*top_k, nch_output=1).to(device)
        elif i == 'posterior_label':
            # Building model
            net = structures.lenet.attacker_mia(nch_info=200, nch_output=1).to(device)
        elif i == 'posterior_correct':
            # Building model
            net = structures.lenet.attacker_mia(nch_info=110 if top_k==None else 10*(top_k+1), nch_output=1).to(device)
        ...
        for ep in range(start_epoch, epochs):
            ...
            # Save checkpoint every 10 epochs
                ...
                save_checkpoint(state, '%s/Epoch_(%d).ckpt' % (model_ckpt_dir, ep), max_keep=2,
                                is_best=False)
                #acc_FM = test_binary(net, information_index, target_index, test_loader_FM)
            #break
