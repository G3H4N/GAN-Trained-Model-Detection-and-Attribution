...
parser.add_argument('--save_name', dest='save_name', type=str, help='save name',
                    default='REAL_multi_GANStrctr_byMstr')
...
dataset_addr_real_byMstr = os.path.join(model_addr, '_datasets_real_byMstr')

content_list = ['posterior_sorted', 'posterior_label', 'posterior_correct', 'Strctr_name']
information_list = ['posterior_sorted', 'posterior_label', 'posterior_correct']
target_list = ['Strctr_name']

for Mstr in Model_structure_list:

    # Datasets preparation
    trainset_path = os.path.join(dataset_addr_real_byMstr, 'GANStrctr_Trainset_real_multiclass_byMstr_'+Mstr)
    trainset = loadData(trainset_path)
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True)
    testset_path = os.path.join(dataset_addr_real_byMstr, 'GANStrctr_Testset_real_multiclass_byMstr_'+Mstr)
    testset = loadData(testset_path)
    test_loader = torch.utils.data.DataLoader(testset, batch_size=args.batch_size, shuffle=True)
    testsetTGAN_path = os.path.join(dataset_addr_real_byMstr, 'GANStrctr_TestsetTGAN_real_multiclass_byMstr_'+Mstr)
    ...
    for i in information_list:
        ...
        for t in target_list:
            ...
            if i == 'posterior_sorted':
                # Building model
                net = structures.lenet.attacker_real(nch_info=800, nch_output=4).to(device)
            elif i == 'posterior_label':
                # Building model
                net = structures.lenet.attacker_real(nch_info=1600, nch_output=4).to(device)
            elif i == 'posterior_correct':
                # Building model
                net = structures.lenet.attacker_real(nch_info=900, nch_output=4).to(device)
