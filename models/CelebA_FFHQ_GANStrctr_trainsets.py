...
for strctr in Strctr_classes_to_index_tensor:
    ...
    for file in filelist:
        if os.path.isdir(os.path.join(save_addr, file)):
            if file.startswith(strctr):
                ...
                elif (not file.endswith('T')) and (not file.endswith('9')):
    ...
    saveData(strctr_dataset_save_dir, 'FFHQ_GANStrctr_Trainset', Trainset_strctr)
...
for data_path_strctr in dataset_strctr_path_list:
    Trainset = loadData(os.path.join(data_path_strctr, 'FFHQ_GANStrctr_Trainset'))
...
saveData(save_addr, 'FFHQ_GANStrctr_Trainset_AllStrctr', Trainset_AllStrctr)
