    ...
    for gan in GANs_list:
        ...
        for model in model_list:
            ...
            for c in range(c_dim):
                ...
                dataset_save_dir = os.path.join(save_addr, 'Trainset_postGAN_multiclass_multiGAN_%05d'%len(dataset_part_path_list))
                TrainSet_part = torch.utils.data.TensorDataset(posterior_sorted, posterior_label, posterior_correct, GAN_name, Strctr_name)
                saveData(save_addr, 'Trainset_postGAN_multiclass_multiGAN_%05d'%len(dataset_part_path_list), TrainSet_part)
                print(len(TrainSet_part))
                del TrainSet_part
...
for data_path_strctr in dataset_part_path_list:
    trainset = loadData(data_path_strctr)
    trainsets_part_list.append(trainset)

TrainSet_AllStrctr = torch.utils.data.ConcatDataset(trainsets_part_list)#torch.utils.data.TensorDataset(posterior_sorted_AllStrctr, posterior_label_AllStrctr, posterior_correct_AllStrctr, GANship_AllStrctr)
saveData(save_addr, 'Trainset_postGAN_together_multiclass_multiGAN_AllStrctr', TrainSet_AllStrctr)
