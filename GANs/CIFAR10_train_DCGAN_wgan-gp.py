...
parser.add_argument('--c_dim', dest='c_dim', type=int, help='# of classes', default=10)
# training
...
# others
parser.add_argument('--experiment_name', dest='experiment_name', default='DCGAN_wgan-gp_0')
...
parser.add_argument('--trainset_size', dest='trainset_size', type=int, default=48000)
...
# save settings
pylib.mkdir('./CIFAR10_outputs/%s' % experiment_name)
with open('./CIFAR10_outputs/%s/setting.txt' % experiment_name, 'w') as f:
...
CIFAR10 = torchvision.datasets.CIFAR10(root='/data/gehan/Datasets/CIFAR10/', train=True, download=True, transform=transform)
split = [trainset_size, len(CIFAR10) - trainset_size]
trainset, _ = torch.utils.data.random_split(CIFAR10, split)
...
# model
D = model.DiscriminatorDCGAN(nch_in=3, n_class=c_dim).to(device)
G = model.GeneratorDCGAN(nch_in=z_dim, n_class=c_dim, nch_out=3).to(device)
...
# load checkpoint
ckpt_dir = './CIFAR10_outputs/%s/checkpoints' % experiment_name
...
# writer
writer = tensorboardX.SummaryWriter('./CIFAR10_outputs/%s/summaries' % experiment_name)

# run
z_sample = torch.randn(c_dim * 10, z_dim, 1, 1).to(device)
c_sample = torch.tensor(np.concatenate([np.eye(c_dim)] * 10), dtype=z_sample.dtype).to(device)
c_sample = c_sample.unsqueeze(-1).unsqueeze(-1)
c_sample_D = c_sample.expand(c_sample.size()[0], c_dim, image_size, image_size)
for ep in range(start_ep, epoch):
    for i, (x, c_dense) in enumerate(train_loader):
        ...
        c_D = c.expand(c.size()[0], c_dim, image_size, image_size)


        x_f = G(z, c).detach()
        x_gan_logit = D(x, c_D)
        x_f_gan_logit = D(x_f, c_D)

        d_x_gan_loss, d_x_f_gan_loss = d_loss_fn(x_gan_logit, x_f_gan_logit)
        gp = model.gradient_penalty_c(D, c_D, x, x_f, mode=gp_mode)
        ...
        if step % 250 == 0:
            ...
            save_dir = './CIFAR10_outputs/%s/sample_training' % experiment_name
