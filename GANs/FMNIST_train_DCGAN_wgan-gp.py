...
# others
parser.add_argument('--experiment_name', dest='experiment_name', default='DCGAN_wgan-gp_6')
...
# save settings
pylib.mkdir('./FM_outputs/%s' % experiment_name)
with open('./FM_outputs/%s/setting.txt' % experiment_name, 'w') as f:
...
'''
FMNIST = dsets.FashionMNIST('/data/gehan/Datasets/FashionMNIST', train=True, download=True, transform=transform)
split = [trainset_size, len(FMNIST) - trainset_size]
trainset, _ = torch.utils.data.random_split(FMNIST, split)
...
# load checkpoint
ckpt_dir = './FM_outputs/%s/checkpoints' % experiment_name
...
# writer
writer = tensorboardX.SummaryWriter('./FM_outputs/%s/summaries' % experiment_name)
...
for ep in range(start_ep, epoch):
    for i, (x, c_dense) in enumerate(train_loader):
        ...
        if step % 250 == 0:
            ...
            save_dir = './FM_outputs/%s/sample_training' % experiment_name
